...
module sa_core #(
    ...
    (* mark_debug = "true" *) output logic [10:0] dma_cnt,
    (* mark_debug = "true" *) output logic [31:0] DATA_OUT,
  ...
  // FSM
 (* mark_debug = "true" *) state_t c_state, n_state;
...
(* mark_debug = "true" *)  logic [ ITER_W-1:0] iter_cnt_S;
(* mark_debug = "true" *)  logic [ ITER_W-1:0] iter_cnt_S_q;   // delayed index for STORE alignment
  ...
  // DPRAM OUTPUT
  (* mark_debug = "true" *) logic [10:0] dpram_out_addra;
  (* mark_debug = "true" *) logic [10:0] dpram_out_addrb;
  logic [31:0] output_data_buffer;
  logic [31:0] dpram_out_dob;

  (* mark_debug = "true" *) logic dpram_out_enb;
  (* mark_debug = "true" *) logic dpram_in_wea;
  (* mark_debug = "true" *) logic dpram_out_wea;
   logic store_done;
  ...
  (* mark_debug = "true" *) logic signed [18:0] sa_out;  // 19-bit accumulator output
  // logic signed [7:0]  data_out;
  logic OUTPUT_EN;
  logic output_en_q;                  // delayed OUTPUT_EN for 1-cycle alignment

  (* mark_debug = "true" *) logic signed [31:0] buff_out;
  ...
  always_ff @(posedge clk or negedge rstn) begin
    if (!rstn) begin
      ...
      dpram_out_wea <= 1'b0;   // STORE 아닐 때는 항상 0
      store_done    <= 1'b0;   // 한 클럭짜리 플래그로 사용
      ...
      case (c_state)
        ...
        S_STORE: begin
          ...
          if (OUTPUT_EN && !store_done) begin
            dpram_out_addra <= iter_cnt_S;
            dpram_out_wea   <= 1'b1;

            if (iter_cnt_S == STORE_WORDS-1) begin
              store_done <= 1'b1;
            end else begin
              iter_cnt_S <= iter_cnt_S + 1;
            end
