...
module sa_core #(
  ...
  // Clamp iter counter to matrix size to avoid DPRAM address overflow
  assign dpram_word_idx = (((iter_cnt >= ELEMS_PER_MATRIX) ? (ELEMS_PER_MATRIX-1) : iter_cnt) / ELEMS_PER_WORD);
  ...
  always_ff @(posedge clk or negedge rstn) begin
    if (!rstn) begin
      ...
      case (c_state)
        ...
        S_WRITE_A: begin
          start_rd_wr   <= 2'b00;
          if (iter_cnt_A < ELEMS_PER_MATRIX) begin
        ...
        S_WRITE_B: begin
          start_rd_wr   <= 2'b00;
          if (iter_cnt_B < ELEMS_PER_MATRIX) begin
